 ...
 *			to pingpong to print out "pong", and sending a signal to ping.c to invoke it's
 *			functionality.
 ***************************************************************************************/
...
long int pingpong;		//to hold pingpong.c's PID
...
void my_handler(int signum)			//handler for signal catching
{
	...
	{
		if(kill(pingpong, SIGUSR2) == -1)	//send signal to pingpong
		{
			perror("Failed to send SIGUSR2 to pingpong OR pingpong process was interrupted");
			if(kill(ping, SIGINT) == -1)		//kill ping process to end program
				perror("Failed to kill ping process OR ping Process is already dead");
			if (raise(SIGINT) != 0)				//kill this process to end the program
   				perror("Failed to raise SIGUSR1");
		}

		if(kill(ping, SIGUSR2))		//send signal to pong
			perror("Failed to send SIGUSR2 to ping");
	}
}
...
int main(int argc, char *argv[])
{
	...
	if(kill(pingpong, SIGUSR1) == -1)				//send signal to pingpong to print out inital "pong"
	{
